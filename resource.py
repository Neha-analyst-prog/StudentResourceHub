...
def download_resource(username):
    ...
    if result:
        ...
        count += 1
        c.execute("UPDATE resources SET download_count = ? WHERE id = ?", (count, resource_id))
        ...
        conn.commit()
        print(f"Downloading '{title}'... (Total Downloads: {count})")
...
def access_shared_resource():
    ...
    if result:
        ...
        if input("\nDownload this resource? (y/n): ").lower() == 'y':
            # Simulate download for anonymous user
            c.execute("UPDATE resources SET download_count = download_count + 1 WHERE id = ?", (resource_id,))
            conn.commit()
